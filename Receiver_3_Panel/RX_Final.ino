...
int previousNumber = -1; // Initialize with an impossible value
unsigned long bellStartTime = 0;
const unsigned long bellDuration = 550; // 1 second bell duration
/*
   Pin Connections
  P10_ENABLE  15  -  PD6
  P10_A       0   -  PA03
...
void loop() {
  ...
  if (status & (1 << 6)) { // RX_DR flag
    byte data[4] = {0};
    readPayload(data, 4);
    receivedNumber = data[0] * 1000 + data[1] * 100 + data[2] * 10 + data[3];
    if (receivedNumber <= 999) {
      dspNum = receivedNumber;
    }
    else{
    ...
    if (dspNum != previousNumber) {
      if (receivedNumber == 0) {
        ...
        digitalWrite(Bell_Relay, HIGH);
        bellStartTime = millis();
        previousNumber = dspNum;
      }

    }
    // Clear RX_DR flag
    writeRegister(STATUS_REG, (1 << 6));
  }

  // Check if bell should be turned off
  if (digitalRead(Bell_Relay) == HIGH && (millis() - bellStartTime) >= bellDuration) {
    digitalWrite(Bell_Relay, LOW);
  }

  P10_display_number(dspNum);
